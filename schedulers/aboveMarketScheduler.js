#!/usr/bin/env node
/**
 * Above Market "Create Forecast" Scheduler
 *
 * This script runs the INITIAL forecast creation process (the long-running operation).
 * This is separate from the ensemble weight updates which remain in the user interface.
 *
 * WHAT THIS DOES:
 *   - Runs the complete "Create Forecast" process (multiple R/Python scripts)
 *   - Takes a long time (potentially hours for large datasets)
 *   - Should be run during off-peak hours via cron
 *
 * WHAT THIS DOESN'T DO:
 *   - Ensemble weight updates (use the web UI for that)
 *   - Those are quick and should remain user-interactive
 *
 * Usage:
 *   node schedulers/aboveMarketScheduler.js
 *
 * Or run via cron:
 *   0 2 * * * cd /var/www/html/cde-ai-app-cps && node schedulers/aboveMarketScheduler.js >> logs/above-market-scheduler.log 2>&1
 */
...
/**
 * Execute the complete Above Market "Create Forecast" process
 * This mirrors the /forecast/above-market endpoint but runs standalone
 */
async function executeAboveMarketForecast() {
    try {
        ...
        // ============================================
        // Step 1: Python - Price Tier Mapping
        // ============================================
        log.info('[Step 1/5] Python: Mapping brands to price tiers...');
        const pythonStartTime = Date.now();

        const pythonResponse = await runPythonScript(
        ...
        // ============================================
        // Step 2: R Script - BGS Data Forecast
        // ============================================
        log.info('[Step 2/5] R Script: Creating forecasts for BGS countries...');
        const r1StartTime = Date.now();

        const r1Response = await runRScript(
            'AboveMarket/BGS/04_Dataprep_BGS.R',
            [AboveMarketInputFilePath, AboveMarketStaticFilePath],
            null // No socket.io for scheduled jobs
        );
        ...
        try {
            const bubbleResponse = await runPythonScript(
